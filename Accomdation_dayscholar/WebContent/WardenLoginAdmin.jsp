...
<% 
		...
		try {
			...
			PreparedStatement ps= con.prepareStatement("select * from ward where name='"+rn+"' and password='"+na+"'");  //sno , name are column name in db in cse table
			ResultSet rs=ps.executeQuery();
			if(rs.next()){
				session.setAttribute("key",rn);//here rn means sno it will be the to check the table
				response.sendRedirect("WardenPortal.jsp");
